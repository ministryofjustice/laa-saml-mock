...
public class SsoControllerTest extends AbstractIntegrationTest {
  ...
  private List<String[]> formParams = singletonList(
  ...
  @Before
  public void setup() {
    addTestUserAttribute();
  }

  private void addTestUserAttribute() {
    Map<String, List<String>> attributes = new HashMap<>();
    attributes.put("test attribute 1", singletonList("test attribute 1 value"));

    given()
      .body(attributes)
      .header("Content-Type", "application/json")
      .put("/api/attributes/testuser")
      .then()
      .statusCode(SC_OK);
  }
  ...
  private void doSingleSignOn(boolean post) throws Exception {
    ...
    assertThat(String.format("SAML response should contain the '%s' attribute", "test attribute 1"), samlResponse.contains("test attribute 1"));
    assertThat(String.format("SAML response should contain the '%s' attribute value", "test attribute 1 value"), samlResponse.contains("test attribute 1 value"));
    assertThat(String.format("SAML response should not contain the '%s' attribute", "other-attribute"), !samlResponse.contains("other-attribute"));
  }
}
